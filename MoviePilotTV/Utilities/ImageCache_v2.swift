...
class ImageCache {
    ...
    func get(forKey key: String) -> UIImage? {
        ...
        if let image = memoryCache.object(forKey: cacheKey) {
            print("✅ [ImageCache] 内存命中: \(key.prefix(40))")
            return image
        }
        
        // 2. 查询磁盘缓存
        if let diskImage = getDiskImage(forKey: key) {
            print("✅ [ImageCache] 磁盘命中: \(key.prefix(40))")
            // 恢复到内存缓存
    ...
    func clearExpiredCache() {
        ...
        guard let urls = try? fileManager.contentsOfDirectory(at: URL(fileURLWithPath: diskCachePath), includingPropertiesForKeys: [.contentModificationDateKey]) else {
            return
        }
        ...
        print("🧹 [ImageCache] 已清除 \(clearedCount) 个过期文件")
    }
}
